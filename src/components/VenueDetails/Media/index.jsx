...
export const Media = ({ mediaArray }) => {
  ...
  return (
    <div
      className={`relative h-[450px] border border-red-500 xl:max-h-[768px] ${mediaArrayLength > 3 ? 'grid-cols-2 gap-2 md:grid' : 'block'} `}
    >
      <div className="h-full w-full border border-blue-600">
        <img
      ...
      {mediaArrayLength > 3 && (
        <div className="hidden min-h-full w-full border border-blue-600 md:grid md:grid-cols-2 md:grid-rows-2 md:gap-1">
          {renderedGallery.map((image, index) => {
            return (
              <img
                src={image?.url}
                key={index}
                className="aspect-square h-full w-full border border-green-600 object-cover sm:rounded-lg"
              />
            );
          })}
        </div>
